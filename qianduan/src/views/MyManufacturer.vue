<template>
 ...
 <div class="mmf">
    ...
    <div class="tab">
      <div class="control">
        <div class="add"> 
          <button @click="goTo()" class="xinzeng">新增</button>
          <button @click='opendata2()' style='width:100px;'>上传出库信息</button>
        </div>
        <div class="cz">        
        <button>导出</button>
      ...
      <el-table-column label="操作" fixed="right" width="180px">
        <template slot-scope="scope">
          ...
          <el-button
            size="mini"
            type="danger"
            @click="delManu(scope.row)">删除</el-button>
        </template>
      </el-table-column>
...
export default {
  ...
  console.log(this.qkl.id)
   ...
   // shenfen(){
    //console.log(this.$userMsg.userType)
     // if(this.$userMsg.userType!=='7'&&this.$userMsg.userType!=='8'){
  ...
  search(){
            axios({
        url:"/api/producerManagement/selectby",
        method:"get",
        params: {
    ...
    loadData(){
      axios({
        url:"/api/producerManagement/page",
        method:"get"
      }).then((response) => {
                  console.log(response)
                  this.tableData=response.data.data;
    ...
    open(row){
       axios({
        url: "/api/ManufacturersInformation/select1/"+ row.id,
        method: "get",
      }).then((response) => {
    ...
    console.log(row);
      ...
      console.log(row);
    axios({
        url:"/api/producerManagement/delete2/"+row.id,
        method:"delete",
      }).then((response) => {
